...
from cdataset import CommitMsgDataset
from model import CommitMessageModel
...
def validate(model, val_generator, device, criterion, need_prob=False):
    ...
    model.eval()
    with torch.no_grad():
        for commit_labels, commit_urls, cc_ids, cc_attention_mask in tqdm(val_generator):
            ...
            outs = model(cc_ids, cc_attention_mask)
...
def predict_test_data(model, test_generator, device, lang="all"):
    ...
    model.eval()
    with torch.no_grad():
        for commit_labels, commit_urls, cc_ids, cc_attention_mask in tqdm(test_generator):
            label_batch = commit_labels.to(device)
            cc_ids = cc_ids.to(device)
            cc_attention_mask = cc_attention_mask.to(device)
            url_batch = commit_urls
    ...
    df.to_csv(f"{lang}_cc.csv", index=False)
...
# def train(model, learning_rate, number_of_epochs, train_generator, val_generator, test_generator=None):
    ...
    # optimizer = AdamW(parameters_list)
    ...
    for epoch in range(number_of_epochs):
        ...
        for commit_labels, commit_urls, cc_ids, cc_attention_mask in tqdm(train_generator):
            label_batch = commit_labels.to(device)
            cc_ids = cc_ids.to(device)
            cc_attention_mask = cc_attention_mask.to(device)
        ...
        if best_val_f1 < val_f1:
            best_val_f1 = val_f1 
            torch.save(model.state_dict(), f"checkpoints-best/msg_finetune_model.pt")
...
def do_train():
    ...
    train_set = CommitMsgDataset(data_path=dataset_name, config=config_train)
    val_set = CommitMsgDataset(data_path=dataset_name, config=config_val)
    test_set = CommitMsgDataset(data_path=dataset_name, config=config_test)
    test_java_set = CommitMsgDataset(data_path=dataset_name, config=config_java_test)
    test_python_set = CommitMsgDataset(data_path=dataset_name, config=config_python_test)
    ...
    model.load_state_dict(torch.load("checkpoints-best/msg_finetune_model.pt"))
