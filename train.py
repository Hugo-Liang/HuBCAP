...
from model import VFHClassifier
...
def validate(model, val_generator, device, criterion, need_prob=False):
    ...
    model.eval()
    with torch.no_grad():
        for commit_labels, code_add_input_ids, code_add_attention_mask, code_delete_input_ids, code_delete_attention_mask, file_attention_mask, hunk_attention_mask, commit_urls, cc_ids, cc_attention_mask in tqdm(val_generator):
            ...
            cc_ids = cc_ids.to(device)
            cc_attention_mask = cc_attention_mask.to(device)
            url_batch = commit_urls
            label_batch = commit_labels.to(device)

            outs = model(cc_ids, cc_attention_mask, code_add_input_ids, code_add_attention_mask, code_delete_input_ids, code_delete_attention_mask,file_attention_mask, hunk_attention_mask)
...
def predict_test_data(model, test_generator, device, lang="all"):
    ...
    model.eval()
    with torch.no_grad():
        for commit_labels, code_add_input_ids, code_add_attention_mask, code_delete_input_ids, code_delete_attention_mask, file_attention_mask, hunk_attention_mask, commit_urls, cc_ids, cc_attention_mask in tqdm(test_generator):
            code_add_input_ids = code_add_input_ids.to(device)
            code_add_attention_mask = code_add_attention_mask.to(device)
            code_delete_input_ids = code_delete_input_ids.to(device)
            code_delete_attention_mask = code_delete_attention_mask.to(device)
    ...
    df.to_csv(f"{lang}_mds50.csv", index=False)
...
def train(model, learning_rate, number_of_epochs, train_generator, val_generator, test_generator=None):
    ...
    for epoch in range(number_of_epochs):
        ...
        for commit_labels, code_add_input_ids, code_add_attention_mask, code_delete_input_ids, code_delete_attention_mask, file_attention_mask, hunk_attention_mask, commit_urls, cc_ids, cc_attention_mask in tqdm(train_generator):
            code_add_input_ids = code_add_input_ids.to(device)
            code_add_attention_mask = code_add_attention_mask.to(device)
            code_delete_input_ids = code_delete_input_ids.to(device)
            code_delete_attention_mask = code_delete_attention_mask.to(device)
        ...
        if best_val_f1 < val_f1:
            best_val_f1 = val_f1 
            torch.save(model.state_dict(), "checkpoints-best/finetunemds50_model.pt")
        earlyStopping(val_loss)

        if (epoch + 1 == FINETUNE_EPOCH) or earlyStopping.early_stop: 
            logger.info("Result on testing dataset...")
            model.load_state_dict(torch.load("checkpoints-best/finetune_model.pt"))
...
def do_train():
    ...
    model = VFHClassifier(torch.load("checkpoints-best/hunk_finetune_model.pt"), torch.load("checkpoints-best/msg_finetune_model.pt"))
    # model.load_state_dict(torch.load("checkpoints-best/finetune_model.pt"))
